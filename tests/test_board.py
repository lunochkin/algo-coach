...
from algo_coach.techniques import latest_claims, map_tags
...
def test_self_labels_are_counted_per_mode():
    attempts = [
        make_attempt("a1", self_label=FailureMode.RUST),
        make_attempt("a2", self_label=FailureMode.RUST),
        make_attempt("a3", self_label=FailureMode.GAP),
    ]
...
def test_an_unlabelled_attempt_counts_toward_the_row_and_no_mode():
    attempts = [make_attempt("a1", self_label=FailureMode.GAP), make_attempt("a2")]
...
def test_a_technique_only_a_claim_names_still_gets_a_row():
    ...
    claims = latest_claims([make_claim(["binary-search"], attempt_id="a2")])
