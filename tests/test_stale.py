...
def test_a_user_claim_is_at_no_configuration():
    ...
    reuses it as a machine claim of its own."""
...
def test_the_readings_are_this_configurations_own_latest():
    ...
    assert machine_claims_at([older, newer], CONFIGURATION, {"a1": PROMPT_HASH})[
        "a1"
    ].techniques == ["sorting"]
...
def test_a_later_claim_from_another_configuration_hides_nothing():
    ...
    on purpose is a rollback, and the machine claim under it still answers."""
    mine = machine_claim("a1", ["greedy"])
    theirs = machine_claim("a1", ["sorting"], prompt_hash="ffffffffffff")

    assert machine_claims_at([mine, theirs], CONFIGURATION, {"a1": PROMPT_HASH})[
        "a1"
    ].techniques == ["greedy"]
...
def test_a_user_claim_is_not_a_reading():
    assert (
        machine_claims_at([user_claim("a1", ["greedy"])], CONFIGURATION, {"a1": PROMPT_HASH}) == {}
    )
