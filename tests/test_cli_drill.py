...
def test_drill_says_it_records_nothing_yet(drill_root, monkeypatch, capsys):
    """The thin loop moves no number; saying so beats a silent no-op."""
    run(monkeypatch, ["1", "1"])

    assert "records nothing yet" in capsys.readouterr().out


def test_technique_flag_skips_the_first_prompt(drill_root, monkeypatch, capsys):
    run(monkeypatch, ["1"], "--technique", "trie")

    assert "Implement Trie — trie" in capsys.readouterr().out


def test_a_bad_choice_is_asked_again(drill_root, monkeypatch, capsys):
    run(monkeypatch, ["nine", "0", "2", "1"])
